...
with
    local_manzana as (
        ...
        from {{db}}_cannibalization.{{report_name}}_{{schema}}_reparto_idpois z
        join {{db}}_cannibalization.{{report_name}}_{{schema}}_delivery_gasto_por_block bg on ST_intersects(ST_GeometryFromText(bg.shape), ST_GeometryFromText(z.shape))
    ...
    ,sp_canibalizacion_reparto as (
        ...
        select
    		...
    		from local_manzana a, 
    			{{db}}_cannibalization.{{report_name}}_{{schema}}_reparto_idpois b,
    			{{db}}_cannibalization.{{report_name}}_{{schema}}_locales_idpois l
    		where a.id_pois = b.pois and CAST(l.pois as int) = a.id_pois and l.marca_gestion = 'Doggis'
    		  and a.id_pois in (
    			select pois
    			from {{db}}_cannibalization.{{report_name}}_{{schema}}_reparto_idpois
    				where ST_intersects(
