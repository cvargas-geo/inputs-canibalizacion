...
def etl_captura(event):
    ...
    try:
        ...
        etl_name    = event.get('input', None).get('etl_name', 'captura')
        report_name = event.get('input', None).get('report_name', 'captura')
        ...
        if stage == 2 and etapa2:
            ...
            """
            print("Step 2 : Tabla de Precalculo ")
            #Nota el generic_path siempre se obtiene a nivel del stage
            generic_path = conf.get_dimanic_sql_path(  etl_name , report_name, stage)
        ...
        if stage == 3 and etapa3:
            ...
            # aca en lugar de cargar locales_propios , ahora es  locales captura
            assert postgres_to_athena('locales_captura', environment , event["input"]) , "Error en etapa 3 al copiar de pg to athena"
            ...
            #es la query de precalculo  locales que le cambie el nombre 
            table_name = 'precalculo_blocks'
            ...
            input_data ={
                "worker_tasks_list": [task_2]
            }
            return input_data

        if stage == 4 and etapa4:
            """
            Fin del proceso de canibalización captura
