...
def etl_delivery(event):
    ...
    try:
        ...
        if stage == 2 and etapa2:
            ...
            if drop_table == False :
                    assert postgres_to_athena(f'reparto_idpois', environment , event["input"] , columns='pois , st_astext(shape) as shape') == True , "Error al obtener la tabla reparto_idpois"
                    assert postgres_to_athena(f'locales_idpois', environment , event["input"] ) == True , "Error al obtener la tabla locales_idpois"
        ...
        if stage == 3 and etapa3:
            ...
            if drop_table == False :
                ...
                df = wr.athena.read_sql_query(f"SELECT * FROM {report_name}_{schema}_{etl_name}_sp_canibalizacion_reparto", database=f"{environment.lower()}_{conf.SERVICE_NAME}")
