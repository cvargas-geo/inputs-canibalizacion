...
def etl_gastos(event ):
    ...
    try:
        ...
        id_gastos = parametros[etl_name][country_name]['id_gastos']
        etapa1=True
        etapa2=True
        etapa3=True
        ...
        if stage: 
            if stage == 1 :
                ...
                generic_path = get_dimanic_sql_path(  'demografico' , customer_name, 2)
                if etapa1 :  
                ...
                if etapa2 :            
                    ...
                    if id_gastos and id_gastos is not None :
                        lista_canastas = get_canastas(
                            f"SELECT COALESCE( name , 'NULO') AS canasta  FROM {DATALAKE_DB}.country_{country_name}_canasta_categoria where id in ({list_to_sql_in(id_gastos)})"
                        )
                    else:
                    ...
                    for i, canasta in enumerate(lista_canastas) :
                        ...
                        custom_table_name = f"{customer_name}_{country_name}_{etl_name}_{table_name}_{canasta.lower()}_b{buffer}"
                        sql_querie =''
                        sql_querie = read_templated_file(
                            f"{generic_path}/02_{table_name}.sql" , 
                            {
                                ...
                                'max_gse':br.get_limit_gse_by_country(country_name)
                            }
                        )
...
# TODO IMPLEMENT
def get_gse_levels(  country_name  ):
    try:
        sql_querie = f"""SELECT
                            COUNT(*) AS income_levels 
                            FROM {DATALAKE_DB}.country_{country_name}_income_levels"""
